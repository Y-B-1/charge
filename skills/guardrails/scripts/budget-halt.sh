#!/usr/bin/env bash
# charge guardrails: budget-halt — PreToolUse hook. Denies tool calls once
# today's estimated spend (via ccusage, cached 60s) crosses
# CHARGE_DAILY_BUDGET_USD. Unset budget = disarmed. Fails OPEN on tooling
# errors (a broken npx must not brick the session) with a stderr warning.
set -uo pipefail
...
if [ "$fresh" -ne 1 ]; then
  if ! npx -y ccusage@latest daily --json > "$CACHE" 2>/dev/null; then
