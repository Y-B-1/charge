...
Delete means delete: write the test first, then fresh code to pass it.
Tempted to keep the old code "as reference"? That IS testing after —
close the file and write the test from the spec.
```
</After>

The positive statement ("start over from the test") leads; the one guardrail
that can't be phrased positively is paired immediately with what to do
instead ("close the file and write the test from the spec").
...
"Don't cheat" doesn't work; a specific counter does.
✅ Fix: For each rationalization, state the positive rule it violates and
pair any remaining guardrail with the do-instead.
